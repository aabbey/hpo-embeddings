...
if __name__ == "__main__":
    ...
    with open('raw_text_inputs/sample_clinician_notes_gpt4_breakdown.txt', 'r') as t:
        notes_breakdown = t.read()
    with open("prompts/doc_note_sysmsg_1.txt", 'r') as p:
        sys_prompt1 = p.read()
    with open("prompts/doc_note_1.txt", 'r') as p:
        prompt1 = p.read()
    ...
    extracted_terms = []
    for line in notes_breakdown.split("\n\n"):
        print(line)
        ch1_hist.append(hm_msg_prompt_template.format_messages(line_and_interp=line)[0])
        further_pheno_analysis = llm(ch1_hist)
        ch1_hist.append(further_pheno_analysis)
        print("\n", further_pheno_analysis)

        term = most_similar(further_pheno_analysis.content)
        print("\n", term)
